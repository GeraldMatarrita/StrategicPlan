...
export class FodaMecaComponent implements OnInit {
  ...
  addCard(columnType: string) {
    Swal.fire({
      title: `Add New ${this.capitalizeFirstLetter(columnType)} Card Analysis`,
      html: `
  ...
  updateCardAnalysis(card:any):void{
    Swal.fire({
      ...
      html: `
        <div>Title</div>
        <input id="swal-input1" class="swal2-input" placeholder="Insert Title" value="${card.title}">
        <div>Description</div>
        <textarea id="swal-input2" class="swal2-textarea" placeholder="Insert description">${card.description}</textarea>
      `,
      focusConfirm: false,
      preConfirm: () => {
        const title = (document.getElementById('swal-input1') as HTMLInputElement).value;
  ...
  deleteCameCard(type: string, card:any): void {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.swotService.deleteCameCard(type, this.planId,card).subscribe(() => {
          this.loadCame()
          Swal.fire('Deleted!', 'Card has been deleted.', 'success');
        });
      }
